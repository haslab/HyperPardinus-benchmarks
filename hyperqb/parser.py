...
def assignVal(key, dict, bitblasting_dict):
	var_name = remove_dot(key)
...
def main_model_parse(smv_file_name, bitblasting_dict, parsed_madel_file_I_name, parsed_madel_file_R_name, PARSE_INDEX):
	...
	smv_file = open(smv_file_name, 'r',encoding="utf-8")
	...
	for state in fsm.pick_all_states(fsm.reachable_states):
		...
		d = disjunct(post_list)
		if d: skip=False; t = trans(getDictAP(curr, bitblasting_dict), d); transitions.append(t); R_bool.write(t)
		else: skip=True
		counter = counter+1
...
def main_formula_construct(formula_file_name, dictionaries, translated_formula_file_name, QS_file_name, To_Negate_formula):
	text = ""
	file = open(formula_file_name, 'r',encoding="utf-8")
	...
	for op in arith_ops:
		...
		text = text.replace(opstar, blasted)
